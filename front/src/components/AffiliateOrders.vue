      ...
      >
        ...
        <template #cell(price)="data">
          ...
          {{
            (
              ...
              parseFloat(data.item.product.price)
            ).toFixed(2)
          }}
        </template>
        <template #cell(comission)="data">
          ...
          {{
            (
              parseFloat(data.item.quantity) *
              parseFloat(data.item.product.comission)
            ).toFixed(2)
          }}
        </template>
...
export default {
  ...
  watch: {
    orders(value) {
      this.items = value.map((order) => {
        delete order.product.image_1;
        delete order.product.image_2;
        delete order.product.image_3;
        return {
          ...order,
        };
      });
